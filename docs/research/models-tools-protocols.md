...
| `events` | action-log entry (append-only) | `type` (e.g. `task.started`, `source.dropped`), `message`, `data` JSON. Every `_emit` writes one (`rootlogic/orchestrator.py:309-312`). The graph engine's `_emit` passes no `data` (`rootlogic/graph.py:396-399`) | `add_event` |
